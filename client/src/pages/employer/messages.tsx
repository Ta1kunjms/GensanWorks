...
import { Loader2, MessageSquare, Send, Search, Circle, RefreshCw, Trash2 } from 'lucide-react';
...
export default function EmployerMessagesPage() {
  ...
  const {
    ...
    queryKey: ['employer', 'messages', 'all'],
    queryFn: async () => {
  ...
  const { data: conversation = [], isLoading: isLoadingConversation, refetch: refetchConversation } = useQuery<Message[]>(
    {
      queryKey: ['employer', 'messages', 'conversation', selectedPeer],
      queryFn: async () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['employer', 'messages', 'all'] });
      queryClient.invalidateQueries({ queryKey: ['employer', 'messages', 'conversation', selectedPeer] });
    })();
  ...
  const deleteConversationMutation = useMutation({
    ...
    onSuccess: async (_data, peerId) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['employer', 'messages', 'all'] });
      queryClient.invalidateQueries({ queryKey: ['employer', 'messages', 'conversation', peerId] });
      ...
      <div className="mx-auto flex h-full min-h-0 max-w-7xl flex-col gap-4 px-6 py-6 box-border">
        ...
        <div className="flex-1 min-h-0">
          <div className="h-full grid gap-4 grid-cols-[340px,1fr] min-h-0">
            ...
            <Card className="h-full flex flex-col border border-slate-200 shadow-sm bg-white">
              ...
              <CardContent className="flex-1 min-h-0 space-y-1 overflow-y-auto overscroll-contain p-2 scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100">
                ...
                {!isLoadingThreads && threads.length === 0 && (
                  <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 p-10 text-center">
                    ...
                    <p className="mt-1 text-sm text-slate-500">Applicants will appear here once you start chatting.</p>
                  </div>
                )}
            ...
            <Card className="h-full min-h-0 flex flex-col border border-slate-200 shadow-sm bg-white">
              {selectedThreadMeta ? (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Avatar className="h-10 w-10 border-2 border-white shadow">
                          <AvatarFallback className="bg-gradient-to-br from-sky-500 to-blue-500 text-sm font-semibold text-white">
                      ...
                      {!isLoadingConversation && conversation.length === 0 && (
                        <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-slate-500">Send your first message to this applicant</p>
                          </div>
                        </div>
                      )}
                      {conversation.map((msg, idx) => {
                        const isMine = msg.senderId?.toString() === currentUserId || msg.senderRole === 'employer';
                        return (
                          <div key={msg.id}>
                              ...
                              <div className={`flex max-w-[70%] items-end gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                                {!isMine && (
                                  <Avatar className="h-8 w-8 flex-shrink-0 mb-5">
                                    <AvatarFallback className="bg-gradient-to-br from-sky-500 to-blue-500 text-xs font-semibold text-white">
                                ...
                                <div className="flex flex-col gap-1">
                                  ...
                                  <div className={`flex items-center gap-1.5 px-1 ${isMine ? 'justify-end' : 'justify-start'}`}>
                                    <span className={`text-xs ${isMine ? 'text-sky-100/90' : 'text-slate-500'}`}>
                    ...
                    <div className="border-t border-slate-200 bg-white p-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          size="icon"
                          className="h-12 w-12 flex-shrink-0 rounded-full bg-sky-600 hover:bg-sky-700"
                        >
                ...
                <div className="flex h-full flex-col items-center justify-center gap-3 text-center">
                  <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-10 py-12">
                    <p className="font-medium text-slate-900">No conversation selected.</p>
                    <p className="mt-1 text-sm text-slate-500">Choose a conversation from the left to view messages.</p>
                  </div>
                </div>
              )}
            </Card>
