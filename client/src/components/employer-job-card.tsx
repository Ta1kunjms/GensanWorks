        ...
        <div className="space-y-2.5 text-sm text-slate-600">
          <div className="flex items-center gap-2.5">
            ...
            <span className="line-clamp-1">
              {job.location || [job.municipality, job.province].filter(Boolean).join(", ") || "No location specified"}
            </span>
          </div>
          <div className="flex items-center gap-2.5">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <span>
              {job.jobStatusPTC === 'P' ? 'Permanent' : job.jobStatusPTC === 'C' ? 'Contractual' : job.jobStatusPTC === 'T' ? 'Temporary' : 'Employment Type N/A'}
            </span>
          </div>
        </div>
      </CardContent>
      <CardFooter className="pt-3 pb-4 px-4 border-t bg-slate-50/50 flex justify-between items-center mt-auto">
        <div className="flex gap-4 text-sm text-slate-600">
          <div className="flex items-center gap-1.5" title="Vacancies">
            ...
            <span className="font-semibold text-slate-900">{job.vacantPositions || 0}</span>
